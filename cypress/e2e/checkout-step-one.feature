Feature: Checkout Process with Multiple Items
  ...
  Scenario: User reviews and proceeds with several items on the cart
  ...
  Scenario: User Proceeds to Checkout, and Encounters Shipping Information Errors
    When the user adds several items to shopping cart:
      | itemName1             | itemName2                |
    ...
    Then the cart should contain the 2 items:
      | itemName1             | itemPrice1 | itemName2                | itemPrice2 |
      | Sauce Labs Bike Light | $9.99      | Sauce Labs Fleece Jacket | $49.99     |
    When the user proceeds to checkout
    Then the user should be on the /checkout-step-one.html page
    When the user fills out the shipping information with:
      | firstName | lastName | zipCode |
